...
if (!function_exists('mb_internal_encoding')) {
    function mb_internal_encoding($encoding = null) { return 'UTF-8'; }
}
// +--------------------------------------------------
// | Header and Globals
// +--------------------------------------------------
...
function utf8_convert($str){
    ...
    if (function_exists('iconv')) {
        ...
        $detected = false;
        // Sadə encoding detection
        if (preg_match('//u', $str)) {
            $detected = 'UTF-8';
        } else {
            $detected = 'ISO-8859-1'; // default
        }
        
        if ($detected == "UTF-8") return $str;
        return @iconv($detected, "UTF-8//TRANSLIT", $str) ?: $str;
    } else {
...
// Əlavə: get_encoding funksiyası yoxdursa
if (!function_exists('get_encoding')) {
    function get_encoding($str) {
        // Sadə encoding detection
        if (function_exists('mb_detect_encoding')) {
            return mb_detect_encoding($str, ['UTF-8', 'ISO-8859-1', 'ASCII'], true);
        }
        // Manual check

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'submit') {
    ...
    $law = isset($data['laws']) ? $data['laws'] : '';
    $cit = isset($data['citizen']) ? $data['citizen'] : '';

    $ok = in_array($cur, array('dram','euro','both'))
       && in_array($law, array('yes','no','undecided'))
       && in_array($cit, array('yes','no'));
...
.radio-card input{position:absolute;opacity:0;pointer-events:none;clip:rect(0,0,0,0);width:1px;height:1px;margin:-1px;overflow:hidden}
...
.rc-dot{width:22px;height:22px;min-width:22px;flex-shrink:0;border:2px solid var(--border2);border-radius:50%;position:relative;transition:all .25s var(--ease);background:var(--card)}
...
.dl-toast{position:fixed;top:24px;right:24px;max-width:400px;background:#fff;border-radius:14px;box-shadow:0 8px 36px rgba(0,0,0,.15),0 2px 8px rgba(0,0,0,.06);padding:18px 40px 18px 18px;display:flex;align-items:flex-start;gap:14px;z-index:3000;transform:translateX(calc(100% + 40px));transition:transform .45s cubic-bezier(.4,0,.2,1);border-left:4px solid #10B981}
.dl-toast.visible{transform:translateX(0)}
.dl-toast-icon{flex-shrink:0;width:26px;height:26px;color:#10B981;margin-top:1px}
.dl-toast-text{font-size:13px;color:#1F2937;line-height:1.55;font-weight:500}
.dl-toast-close{position:absolute;top:10px;right:12px;background:none;border:none;cursor:pointer;color:#9CA3AF;font-size:20px;line-height:1;padding:4px;transition:color .2s}
.dl-toast-close:hover{color:#111827}
@media(max-width:768px){.dl-toast{top:auto;bottom:16px;left:16px;right:16px;max-width:none;transform:translateY(calc(100% + 40px))}.dl-toast.visible{transform:translateY(0)}}
    </style>
</head>
...
function triggerDownload(token){var iframe=document.createElement('iframe');iframe.style.display='none';iframe.src='?action=dl&t='+encodeURIComponent(token);document.body.appendChild(iframe);setTimeout(function(){if(iframe.parentNode)iframe.parentNode.removeChild(iframe)},60000);showDlToast()}
...
function showDlToast(){var dict=T[currentLang]||T.en;var txt=dict.toast_download||T.en.toast_download;document.getElementById('dlToastText').textContent=txt;var toast=document.getElementById('dlToast');toast.classList.add('visible');if(toast._timer)clearTimeout(toast._timer);toast._timer=setTimeout(function(){toast.classList.remove('visible')},8000)}
function initDlToast(){document.getElementById('dlToastClose').addEventListener('click',function(){var toast=document.getElementById('dlToast');toast.classList.remove('visible');if(toast._timer)clearTimeout(toast._timer)})}
...
document.addEventListener('DOMContentLoaded',function(){var saved=null;try{saved=localStorage.getItem('lang')}catch(e){}applyLanguage(saved||'hy');initLangSwitcher();initHeader();initForm();initModal();initDlToast();initScrollAnimations()});
})();
</script>
</body>
</html>
